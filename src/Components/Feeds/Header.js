...
function Header({userDocumentData, otherUsers}) {
  ...
  const handleOnChange = (e) => {
    ...
    const uploadTaskListener = storage.ref(`/posts/${userDocumentData.userId}/${file.name}`).put(file);
    ...
    async function fn3() {
      try {
        ...
        let obj = {
          ...
          pId: id,
          pUrl: url,
          uName: userDocumentData?.username,
          uProfile: userDocumentData?.profileUrl,
        ...
        await database.users.doc(userDocumentData.userId).update({
          postIds: [...userDocumentData.postIds, docref.id]
        })
        setLoading(false)
      } catch (e) {
        setError(e);
